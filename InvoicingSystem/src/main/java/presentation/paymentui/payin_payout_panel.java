...
public class payin_payout_panel extends JPanel{
	int is_a_customer=0;
	...
	String situation="in";
	paymentblservice pv=new paymentcontroller("in");
	
	JRadioButton inbutton=new JRadioButton("收款",true);
	JRadioButton outbutton=new JRadioButton("付款",false);
	ButtonGroup group=new ButtonGroup();
	
	JLabel l1=new JLabel("客户名称");
	JLabel l2=new JLabel("账户名称");
	JLabel l3=new JLabel("转账金额");
	JLabel l4=new JLabel("备注");
	JLabel l5=new JLabel("转账列表");
	...
	public payin_payout_panel(String s){
		...
		panel.add(inbutton);panel.add(outbutton);
		...
		button3.setBounds(260, 150, 80, 30);button3.setVisible(true);
		...
		l1.setBounds(270, 50,100, 50);l1.setVisible(true);
		...
		f1.setBounds(200, 100, 200,30);f1.setVisible(true);
		...
		inbutton.setBounds(100,50, 100,50);inbutton.setVisible(true);
		outbutton.setBounds(100,130, 100,50);inbutton.setVisible(true);
		
		js.setBounds(100, 220, 800, 230);js.setVisible(true);
		
		f3.addKeyListener(new numberlistener());
		inbutton.addItemListener(new inbuttonlistener());
		outbutton.addItemListener(new outbuttonlistener());
	...
	public class button1listener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			int i=0;
			if(is_a_customer==0){
				JOptionPane.showMessageDialog(null, "必须有一个客户");
			}
			else{
			...
			if(i==1){
				...
				is_a_customer=0;
				if(situation.equals("in")){
					pv=new paymentcontroller("in");
				}
				else if(situation.equals("out")){
					pv=new paymentcontroller("out");
				}
    ...
    public class inbuttonlistener implements ItemListener{
    ...
    public class outbuttonlistener implements ItemListener{
    ...
    public class button3listener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(f1.getText().trim().length()==0){
				JOptionPane.showMessageDialog(null, "请输入客户名");
			}
			else{
				Member m=new Member();
				ArrayList<MemberVO> v=m.findmember(f1.getText(), "");
				...
				if(v.size()==0){
					...
					pv.addcustomer(v.get(0).getName());
    ...
    public class button4listener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(f2.getText().trim().length()==0||f3.getText().trim().length()==0){
				JOptionPane.showMessageDialog(null, "账户名或金额不能为空");
			}
			else{
				accountcontroller c=new accountcontroller();
				String s=c.find(f2.getText());
				if(s.equals("不存在;-1.0")){
					JOptionPane.showMessageDialog(null, "账户不存在");
				}
				else{
					accountlineitemVO v=new accountlineitemVO(f2.getText(),Double.parseDouble(f3.getText()),f4.getText());
					int i=pv.addaccountlist(v);
					if(i==1){
    ...
    public class button6listener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			panel0.add(button01);
			System.out.println("int action:add button01");
			panel0.add(button02);
			panel0.add(button03);
		...
		public class button01listener implements ActionListener{
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				System.out.println("int button01 action: per");
